...
const searchCollections = catchAsync(async (req, res) => {
  ...
  const data = await collectionService.searchCollections(query);
  res.send(data);
});
...
const getHoldersActions = catchAsync(async (req, res) => {
  const { network } = req.query;
  const { address } = req.params;
  const data = await collectionService.getHoldersActions(network, address);
  res.send(data);
});
