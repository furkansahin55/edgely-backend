...
const getMintingTable = async (network, minutes) => {
  try {
    ...
    const result = await sequelize.query(
      `
      WITH prior_frame AS (
        SELECT address, COUNT(price_as_eth) as mints, COUNT(DISTINCT (to_address)) as minters, SUM(price_as_eth) as volume FROM ${network}.nft_mints 
...
const getMintingLabelTable = async (network, minutes, user) => {
  try {
    ...
    const result = await sequelize.query(`
		WITH wallets AS (
			SELECT l1.address as to_address FROM labels l1 WHERE l1.user = $2 AND l1.type = 0
			UNION ALL
			SELECT t.to_address FROM ${network}.nft_tokens t WHERE t.address IN (SELECT l2.address FROM labels l2 WHERE l2.user = $2 AND l2.type = 1)
		)
		, prior_frame AS (
